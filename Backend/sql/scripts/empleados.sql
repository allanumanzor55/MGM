DELIMITER /
CREATE OR REPLACE PROCEDURE guardarEmpleado(IN TipoEmpleado INT, IN Dni TINYTEXT,
...
BEGIN 
    ...
    VALUES(TipoEmpleado, Nombre, Dni, PrimerApellido, SegundoApellido, Direccion, Correo, Celular, Telefono, Sueldo); 
END/
...
BEGIN
    SELECT * FROM empleados WHERE idEmpleado = Id AND estado = 1;
END/

CREATE OR REPLACE PROCEDURE obtenerEmpleadosPorTipo(IN Tipo INT )
BEGIN
    SELECT * FROM empleados WHERE tipoEmpleado = Tipo AND estado = 1;
END/
